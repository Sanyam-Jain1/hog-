...
def free_bacon(score):
    ...
    cubed_score = score**3
    list = []
    for i in str(cubed_score):
        list.append(i)
    length = len(list)

    if length%2==0:
            return 1 + abs(cubed_score//100000 - (cubed_score%100000)//10000 + (cubed_score%10000)//1000 - (cubed_score%1000)//100 + (cubed_score%100)//10 - cubed_score%10)
    else:
            return 1 + abs((cubed_score%100000)//10000 - (cubed_score%10000)//1000 + (cubed_score%1000)//100 - (cubed_score%100)//10 + cubed_score%10)
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    score = 3**(player_score + opponent_score)
    list = []
    for i in str(score):
        list.append(i)
    length = len(list)
    if length>1:
        if score//(10**(length-1))==score%(10):
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who==0:
            ...
            if current_score0!=0:
                if abs(current_score0 - num_rolls0) == 2:
                    score0+=3
            current_score0 = take_turn(num_rolls0, opponent_score, dice)
            score0+=current_score0
            if abs(num_rolls0 - current_score0) == 2:
                score0+=3
            ...
            num_rolls1 = strategy1(score, opponent_score)
            if abs(num_rolls1 - current_score1) == 2:
                score1+=3
            current_score1 = take_turn(num_rolls1, opponent_score, dice)
